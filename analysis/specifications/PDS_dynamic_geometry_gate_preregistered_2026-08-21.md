...
1. The same 12 January 2008 COR2 anchors, the same traced COR1 path, and the
   same bias/exposure-normalized Level-0-derived total-B and signed-pB cubes
   used in the frozen event-phase analysis are retained.
2. The independent 2008-07-26 23:30 UT event and both processed July
...
periodic test still requires at least 70% direct coverage. This quality rule
was fixed after inspection of the data coverage and before evaluation of the
resulting statistical outcomes.

## Single-event exploratory analysis

After completion of the frozen population tests, a separate exploratory
single-event analysis was retained to examine whether a physically interesting
example could be identified without requiring the full-series threshold. The
strongest already identified individual ridge may therefore be described as a
*single-event candidate*. This post-primary analysis retains the individual raw
probabilities, product failures, and comparison caveat and does not alter the
population-level conclusions above.